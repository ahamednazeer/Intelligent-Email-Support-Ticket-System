...
import { Headset, Lock, User } from '@phosphor-icons/react';
...
export default function LoginPage() {
    ...
    useEffect(() => {
        async function checkExistingAuth() {
            try {
                ...
                const route = userData.role === 'ADMIN'
                    ? '/admin'
                    : ['SUPERVISOR', ...agentRoles].includes(userData.role)
                        ? '/dashboard/agent'
                        : '/';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const route = response.user.role === 'ADMIN'
                ? '/admin'
                : ['SUPERVISOR', ...agentRoles].includes(response.user.role)
                    ? '/dashboard/agent'
                    : '/';
            router.push(route);
        } catch (err: any) {
            ...
            <div className="relative z-10 w-full max-w-md mx-4">
                <div className="bg-slate-900/90 border border-slate-700 rounded-sm p-8 backdrop-blur-md">
                    <div className="flex flex-col items-center mb-8">
                        <Headset size={48} weight="duotone" className="text-blue-400 mb-4" />
                        <h1 className="text-3xl font-chivo font-bold uppercase tracking-wider text-center">
                            Support Ticket System
                        </h1>
                        <p className="text-slate-400 text-sm mt-2">Intelligent Email Support Console</p>
                    </div>
                    ...
                    <div className="mt-6 p-4 bg-slate-950/50 border border-slate-800 rounded-sm">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs font-mono text-slate-400">
                            ...
                            <div>Technical: agent1 / agent123</div>
                        </div>
                    </div>
                </div>
            </div>
